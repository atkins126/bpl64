...
begin
   ...
   if MatchText(ExtractFileExt(GetModuleName()).ToUpper(), ['.DLL', '.BPL']) then
      Exit;
      ...
      begin
         try
            repeat
               ...
               if MatchText(ExtractFileExt(strPathDll).ToUpper(), ['.DLL', '.BPL']) then
...
begin
   ...
   intHandle := 0;
   if ExtractFileExt(PWideChar(strFileName)).ToUpper() = '.DLL' then
      intHandle := LoadLibrary(PWideChar(strFileName))
   else if ExtractFileExt(PWideChar(strFileName)).ToUpper() = '.BPL' then
